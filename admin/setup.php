...
if($button_next) $step++;
//if((($step == 2) && $button_next) && (md5($licence_mail.'AWF1') != $key)) { $step = 1; }
...
if($step == 1) { 
	...
	if((($step == 1) && $button_next) && (md5($licence_mail.'AWF1') != $key)) { 
		echo '<b><font color="#aa0000">Your licence key is incorrect.</b></font><br /><br />'; }
	?>
	...
	<br><br>
	<input type="checkbox" name="register" value="yes"> Register this copy of AWF (no more information must 
be provided)<br>
...
?>');
	...
	if(!defined('WEBMASTER_MAIL')) define('WEBMASTER_MAIL', $SERVER_ADMIN);
	if(!defined('SITE_TITLE')) define('SITE_TITLE', $SERVER_NAME);
	?>
	...
	<input type="text" name="serverpath" size="40" width="40" value="<? echo
		strrev(strstr(substr(strstr(strrev($PATH_TRANSLATED),'/'), 1),'/')); ?>"><br><br>
	Default design<br>
	...
	<input type="checkbox" name="update_db" value="1" /> Update existing database from 1.00 to 1.03 
...
elseif($step == 4) { 
	...
	if($install_phorum == '1') {
		$fcontents = file ($phorum_filename);
		while(list($id, $line) = each($fcontents)) {
	       		if($line && $line[0] != '#') $sql_dump .= $line;
			if(strpos($line,';')) {
				$sql_dump[strlen($sql_dump)-2] = ' ';
	...
	$wildcards['webmaster'] = $webmastermail;
	$wildcards['path'] = 'http://'.$SERVER_NAME.dirname(dirname($REQUEST_URI)).'/apps/phorum';
	$wildcards['host'] = $db_host;
	$wildcards['database'] = $db_name;
	$wildcards['user'] = $db_username;
	$wildcards['passwd'] = $db_password;

	$fd = fopen ('../apps/phorum/admin/settings/forums.awf', 'rb');
	$content = replace_wildcards (fread ($fd, filesize('../apps/phorum/admin/settings/forums.awf')), $wildcards);
	fclose($fd);

	$fd = fopen ('../apps/phorum/admin/settings/forums.php', 'w');
	fwrite($fd, $content);
	fclose($fd);
