...
if(!isset($order_by) || $order_by == '') $order_by='user_id DESC';

if(isset($search)) { 
	$where = 'WHERE '.TABLE_USERS.'.id = user_id AND email LIKE "%'.$search.'%"'; 
	}
else {
	$where = 'WHERE '.TABLE_USERS.'.id = user_id'; 
	}
...
elseif($action=='delete' && isset($id)) {
	...
	echo '<h3><a href="'.$PHP_SELF.'?first_item='.$first_item.'">No</a>&nbsp;
		<a href="'.$PHP_SELF.'?first_item='.$first_item.'&delete='.$id.'">Yes</a></h3>';
	}
elseif($action=='new') {
		echo '<h3>Add new user</h3><form action="'.$PHP_SELF.'" method="post">';
...
elseif($action=='edit' && isset($id)) {
	...
	if(sql_num_rows($qres) > 0) {
		...
		echo '<h3>Edit user data (UserID '.$row[0].')</h3><form action="'.$PHP_SELF.'" method="get">';
		echo '<input type="hidden" name="action" value="edit">';
		...
		while(list($key, $value) = each($profile)) {
			echo '<tr><td width="100" bgcolor="#eeeeee">'.$key.'</td>
				...
				<a href="'.$PHP_SELF.'?action=edit&id='.$id.'&save=remove_userkey&prof_key='.$key.'">Remove</a></td></tr>';
			}
		echo '</table>';
		}
		echo '<form action="'.$PHP_SELF.'" method="get"><br>Key<br><input type=text name="prof_key" size="70"><br>
			<input type="hidden" name="action" value="edit">
                	<input type="hidden" name="id" value="'.$id.'">
	...
	<?
	$qres = sql_query("SELECT count(*) FROM ".TABLE_USERS);
	$row  = sql_fetch_row($qres);
        $total_count = $row[0];
	sql_free_result($qres);
	$qres = sql_query("SELECT DISTINCT user_id, email, password, valid, views, last_login FROM ".TABLE_USERS.", 
		".TABLE_USERDATA." $where"." ORDER BY $order_by LIMIT $first_item, $inc_value");
        echo '<center>';
        echo '<input type=hidden name="order_by" value="'.$order_by.'">';
	...
	<table width="100%" border="0" cellpadding="2" cellspacing="1">
        ...
        <td bgcolor="#ddffdd" width="7%" align="center">
                <a href="<?=$PHP_SELF?>?first_item=<? echo $first_item; ?>&order_by=user_id%20DESC"><b>ID</b></a></td>
        <td bgcolor="#ffdddd" width="61%">
                <a href="<?=$PHP_SELF?>?first_item=<? echo $first_item; ?>&order_by=email"><b>eMail</b></a></td>
<?
/*
	<td bgcolor="#ffdddd" width="20%">
                <a href="<?=$PHP_SELF?>?first_item=<? echo $first_item; ?>&order_by=password"><b>Password</b></a></td>
*/
?>
	<td bgcolor="#ddddff" width="6%" align="center">
                <a href="<?=$PHP_SELF?>?first_item=<? echo $first_item; ?>&order_by=valid"><b>Valid</b></a></td>
        <td bgcolor="#ddddff" width="6%" align="right">
                <a href="<?=$PHP_SELF?>?first_item=<? echo $first_item; ?>&order_by=views%20DESC"><b>Views</b></a></td>
        <td bgcolor="#ffddff" width="10%" align="right">
                <a href="<?=$PHP_SELF?>?first_item=<? echo $first_item; ?>&order_by=last_login"><b>Last Login</b></a></td>
...
?>
		...
		<a href="<?=$PHP_SELF?>?id=<? echo $row[0]; ?>&action=delete&first_item=<? echo $first_item; ?>"><img
			src="../img/delete.gif" border="0" alt="Delete"></a>&nbsp;
		<a href="<?=$PHP_SELF?>?id=<? echo $row[0]; ?>&action=edit&first_item=<? echo $first_item; ?>"><img
...
?>
	...
	if($total_count > $inc_value) {
           ...
           while($inc_value + $total_count > $temp_counter) {
            ...
            echo '<a href="'.$PHP_SELF.'?id='.$id.'&first_item='.($temp_counter - $inc_value).'">'.$news_counter.'</a> ';
            if($first_item == ($temp_counter - $inc_value)) { echo '</b>'; }
            $temp_counter += $inc_value;
            if($news_counter >= 45) { break; }
            }
           if($total_count > $first_item + $inc_value) {
            echo '&nbsp;<a href="'.$PHP_SELF.'?id='.$id.'&first_item='.($first_item + $inc_value).'">Next</a>';
            }
           echo '<br />';
	   }
	 }
	echo '<br /><a href="'.$PHP_SELF.'?action=new">Add new user</a>';
