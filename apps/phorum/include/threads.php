...
<?php
  ...
  while (is_array($message)){
    ...
    if( $$phcollapse != 0 && !$read ){
      ...
      $trec=next($threads);
      echo '  <td class="PhorumListRow" align="center" width="80" '.$color.' nowrap>'.$t_count."&nbsp;</td>\n";
      echo '  <td class="PhorumListRow" width="120" '.$color.' nowrap>'.$t_latest."&nbsp;</td>\n";
    }
    else{
      echo '  <td class="PhorumListRow" width="120" '.$color.' nowrap>'.$t_datestamp.'&nbsp;</td>'."\n";
    }
    echo "</tr>\n";
    $x++;
    $loc++;
